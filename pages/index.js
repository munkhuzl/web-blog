...
function loadMore() {
  ...
  async function loadInitialArticles(){
    setLoading(true);
    const response = await fetch(`https://dev.to/api/articles?username=vyan&page=${selectedCategorypage}&per_page=1`);
        ...
        </div>
        {
          !ended(
            <div className="py-16 text-center">
              <button disabled={loading} className="btn btn-lg btn-accent" onClick={loadNextArticles}>
                Load more
              </button>
            </div>
          )
